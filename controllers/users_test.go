...
func TestUserPutWithCorrectData(t *testing.T) {
	...
	Helper = MockHelper{
		...
		MockGenerateRandomStringStrResult:      "123456",
	}
...
func TestUserPostWithCorrectData(t *testing.T) {
	...
	userdetails.PasswordHash = ""                                       // passHash at DB/cache
	...
	dbmock.ExpectQuery("SELECT \\* FROM authentication WHERE id = ?").WithArgs(expectedUser.ID).WillReturnRows(rows)
	dbmock.ExpectBegin()
	dbmock.ExpectExec("UPDATE `authentication`").WillReturnResult(sqlmock.NewResult(1, 1))
	expectedUser2 := models.Authentication{
		ID:                    1,
		AccountType:           "user",
		CompanyID:             2,
		DOB:                   "2023-10-05",
		...
		PasswordHash:          "",
		Status:                "active",
		IsWizardComplete:      "personal",
	}
	rowss := sqlmock.NewRows([]string{"id", "name", "email", "passwordHash", "twoFactorKey", "twoFactorRecoveryCode", "dob", "accountType", "companyId", "status", "iswizardcomplete"}).AddRow(expectedUser2.ID, expectedUser2.Name, expectedUser2.Email, expectedUser2.PasswordHash, expectedUser2.TwoFactorKey, expectedUser2.TwoFactorRecoveryCode, expectedUser2.DOB, expectedUser2.AccountType, expectedUser2.CompanyID, expectedUser2.Status, expectedUser2.IsWizardComplete)

	// I am expecting a db op before final updation to get user
	dbmock.ExpectQuery("SELECT \\* FROM authentication WHERE email = ?").WillReturnRows(rowss)
	dbmock.ExpectExec("UPDATE `authentication`").WillReturnResult(sqlmock.NewResult(1, 1))
	// expecting a commit to as this is correct info
	dbmock.ExpectCommit()
